...
for cnt_trigger = 1 : 1%size(shift, 1)
    %Shifting triggers by delay
    triggers_AC21 = double(triggers_AC21) + double(round(shift(cnt_trigger, 1) * fs));
    triggers_AC22 = double(triggers_AC22) + double(round(shift(cnt_trigger, 2) * fs));
    triggers_AC23 = double(triggers_AC23) + double(round(shift(cnt_trigger, 3) * fs));
    %%
...
%         end
%     fun = reshape(data_mat, [size(data_mat, 1) * size(data_mat, 2), size(data_mat, 3)]);
%     for electrode = 1 : size(fun, 2)
%         fig = figure();
%         plot(fun(:, electrode));
%     end
    %%
