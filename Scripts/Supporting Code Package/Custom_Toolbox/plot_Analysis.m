...
function [psd_mat] = plot_Analysis(data, triggers, events, window_mrcp, ...
    ...
    for cnt = 1 : length(unique(events))
        plotErdsMap(calcErdsMap([data(:, 7) data(:, 11) data(:, 9)], ...
            erds_info, window_erds, f_borders, 'ref', ref_window, 'sig', 'boot', 'class', ...
            classes(cnt), 'alpha', 0.01, 'montage', [1 1 1]), ....
            fullfile('../Plots/', strcat('ERDS map for patient ', patient, '_class_', string(classes(cnt)))));
    end
    
    %Calculating bandpower and vizualising it
    psd_mat = plot_Bandpower(data, triggers, unique(events), events, ...
    ...
    plot_MRCP(data, triggers, unique(events), events, ...
        channels, window_mrcp, fs, ...
