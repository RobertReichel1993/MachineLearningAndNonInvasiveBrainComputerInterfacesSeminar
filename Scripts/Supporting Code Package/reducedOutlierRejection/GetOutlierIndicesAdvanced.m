...
function [aOutCHIdc, aOutEpochIdc] = GetOutlierIndicesAdvanced ( aSigRaw, aCurTriggers, fs, START_ART_CHECK_SAMPLE, END_ART_CHECK_SAMPLE, varargin )
  ...
  VARARGIN_VARIABLES = { 'FILTER_BAND', 'FILTER_ORDER', 'EP_AMP_TH_MIN', 'EP_AMP_TH_MAX' };
  
  ASSIGN_VARARGIN_FIELDS_TO_VARIABLES

  
  %% (1) Filter signal prior to outlier rejection
  [bb, aa] = butter ( FILTER_ORDER, FILTER_BAND ./ ( fs / 2 ) );  
  
  % Apply filter to channels
  aSigBPass = filter ( bb, aa, aSigRaw );
  ...
  aJP_EpochRej = find ( sum ( aJP_EpochRejMat, 1 ) > 0 );
  ...
  aKU_EpochRej = find ( sum ( aKU_EpochRejMat, 1 ) > 0 );
  ...
  %% (6) Channel rejection by probability (raw)
